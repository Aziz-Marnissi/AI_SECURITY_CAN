# 🛡️ SecureAI_IDS — TinyML Intrusion Detection for CAN Bus (ESP32)

A lightweight, edge-deployed AI system that detects anomalies and attacks on an automotive CAN bus network, using a quantized and structurally pruned neural network running on an ESP32 microcontroller via TensorFlow Lite Micro.

End-of-studies project exploring the full TinyML deployment pipeline — from synthetic CAN traffic generation, through training, structural pruning, and int8 quantization, to real hardware validation on ESP32 (accuracy, RAM, flash, and inference latency measured on-device, not estimated).

## 🧭 Table of contents
- [🏗️ System architecture](#1--system-architecture)
- [📊 The dataset](#2--the-dataset)
- [🧠 The AI model — theory](#3--the-ai-model--theory)
- [💻 Results — host-side evaluation](#4--results--host-side-evaluation-all-3-model-variants)
- [⚡ Results — real ESP32 hardware](#5--results--real-esp32-hardware-ground-truth)
- [📈 Plots explained](#6--plots-explained)
- [📁 Repository structure](#7--project-structure)
- [⚙️ Setup & reproduction](#8--setup--reproduction)
...
2. 🏋️ **Training** — a small MLP (multilayer perceptron) trained to classify traffic into 5 classes
3. ✂️ **Pruning** — structural pruning to shrink the network's neuron count
4. 🔢 **Quantization** — converting float32 weights to int8 for embedded deployment
5. 🔌 **Deployment** — the quantized model is compiled into a C array and run on an ESP32 via TensorFlow Lite Micro
...
Final dataset: **7,980 rows** (6,000 normal + ~500 per attack type), split 80/20 train/test with stratification (`train_test_split(..., stratify=y)`), so each class is proportionally represented in both sets.
...
```
This is **post-training integer quantization** using a representative dataset (300 real samples) so the converter can calibrate realistic min/max ranges per layer.
...
**✂️ (a) Unstructured (magnitude) pruning** — via `tensorflow_model_optimization` (`tfmot`). Individual weights below a magnitude threshold are zeroed out, following a polynomial sparsity schedule (0% → 60% sparsity over training). This keeps the architecture shape (16→8) unchanged, just with many zeroed connections.

> ⚠️ **Key finding**: TFLite's default flatbuffer format stores every weight *including zeros* — sparsity alone does **not** shrink the `.tflite` file. It only helps with **compression** (e.g. `.gz` for OTA transfer: our model compressed from 3.41KB → 1.52KB, a 55% reduction) and can help RAM/latency slightly via zero-skipping in some kernels.

**✂️ (b) Structural pruning (neuron removal)** — the theoretically correct technique for shrinking microcontroller models. Instead of zeroing individual weights, entire neurons are removed:

| Step | What happens |
|---|---|
| 1️⃣ Rank | Rank each hidden-layer neuron by the **L1 norm** of its weights + bias (a proxy for how much it contributes to the network's output) |
| 2️⃣ Select | Keep only the top-k neurons per layer, discard the rest |
| 3️⃣ Transplant | Copy the kept neurons' weights into a smaller network (16→8 becomes 8→4) |
| 4️⃣ Fine-tune | Retrain briefly to recover accuracy lost from the size reduction |
...
| 🗜️ Model size (gzip) | — | 1.52 KB (−55.4% vs raw) | — |
| 🧠 Estimated RAM (tensor arena) | 1.23 KB | 0.39 KB (−68%) | **0.18 KB (−85%)** |
| ⏱️ Host CPU latency (proxy only) | 0.0028 ms | 0.0028 ms | 0.0029 ms |
...
├── export_val_samples_c.py                                                     # export validation samples as C array
└── check_pipeline.sh                                                             # sanity-check script
```
...
```
Verifies the pipeline script contains pruning code, that all expected model/data files exist, and that the pruned model was actually regenerated after the base model.

---

## 9. ✅ Key Takeaways

1. ⚠️ **Unstructured (magnitude) pruning does not reduce on-device flash/RAM** unless paired with sparse storage or compression — a common misconception. It's still useful for OTA transfer size (gzip) and can marginally help latency.
2. ✂️ **Structural pruning (neuron removal) is the correct technique for microcontroller deployment** — it physically shrinks the model's tensors, giving real reductions in flash, RAM, and compute.
3. 🏋️ **Fine-tuning after structural pruning is essential** — accuracy dropped to 89.47% immediately after neuron removal, but recovered to ~99% after 20 epochs of fine-tuning.
...
## 🛠️ Stack
TensorFlow / Keras · TensorFlow Model Optimization (tfmot) · TensorFlow Lite Micro · PlatformIO · ESP32 (Arduino framework) · Python · NumPy · scikit-learn · Matplotlib

## 👤 Author
**Aziz Marnissi** — Electrical Engineering, ENIT (GE, SMART track) · TinyML / Edge AI Security project (CAN bus intrusion detection on ESP32)
